...
CDDIS_OBS_BASE_URL = os.getenv("CDDIS_OBS_BASE_URL", "https://cddis.nasa.gov/archive/gnss/data/daily/")
...
def https_download_file(url):
  ...
  # CURL_NETRC_OPTIONAL (1) = use .netrc credentials when the server
  # asks for auth, ignore otherwise. Safe for all URLs.
  crl.setopt(pycurl.NETRC, 1)
  # Read cookies back so the OAuth redirect chain works
  crl.setopt(crl.COOKIEFILE, '/tmp/cddis_cookies')
...
def download_cors_station(time, station_name, cache_dir):
  ...
  try:
    ...
    logging.info(
      f"Station {station_name} not found on US CORS servers, "
      f"trying CDDIS for international IGS stations..."
    )

  # ── Step 2: try NASA CDDIS archive (works for all ~530 IGS stations
  #    worldwide — requires Earthdata credentials in ~/.netrc) ──
  # CDDIS daily RINEX URL structure:
  #   https://cddis.nasa.gov/archive/gnss/data/daily/YYYY/DDD/YYd/ssssdddf.YYd.gz
  cddis_folder_path = t.strftime('%Y/%j/%yd/')
  cddis_filename = station_name + t.strftime("%j0.%yd")
  # Compression format changed around late 2020: .gz for newer, .Z for older
  ...
  # If .gz failed for recent data, also try .Z as some stations vary
  ...
  logging.warning(
    ...
    f"Check station name and date availability, and verify that "
    f"~/.netrc contains NASA Earthdata credentials."
  )
  return None
