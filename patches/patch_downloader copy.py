...
def patch(source: str) -> str:
    ...
    # ──────────────────────────────────────────────────────────────
    # PATCH 0: Ensure 'import logging' exists at the top of the file
    # ──────────────────────────────────────────────────────────────
    if '\nimport logging\n' not in patched and not patched.startswith('import logging\n'):
        # Insert after the last existing import line
        last_import = None
        for m in re.finditer(r'^(?:import |from \S+ import )', patched, re.MULTILINE):
            last_import = m
        if last_import:
            # Find end of that import line
            line_end = patched.index('\n', last_import.start()) + 1
            patched = patched[:line_end] + 'import logging\n' + patched[line_end:]
            print("  [0/4] Added missing 'import logging'")
        else:
